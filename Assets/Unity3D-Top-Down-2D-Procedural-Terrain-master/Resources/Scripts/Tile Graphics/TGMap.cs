...
public class TGMap : MonoBehaviour
{
    ...
    public static TGMap current;

    public List<GameObject> Listaobj = new List<GameObject>();
    ...
    void Start()
    {
        ...
        foreach (GameObject obj in Listaobj)
        {
            Destroy(obj);
        }
        Listaobj.Clear();
    ...
    void Update()
    {
        ...
        {
            BuildTexture2(transform.position.x, transform.position.z);
    ...
    {
        // Declare mesh points
        int numTiles = mapSizeWidth * mapSizeHeight * 5;
	...
	void BuildTexture()
	{
		TDMap map = new TDMap(mapSizeWidth, mapSizeHeight, minRoomSize, maxRoomSize, totalRooms, seed, noiseFactor, 0 , 0);
    ...
    void BuildTexture2(float xx, float zz)
    {
        TDMap map = new TDMap(mapSizeWidth, mapSizeHeight, minRoomSize, maxRoomSize, totalRooms, seed, noiseFactor, xx, zz);

        int textWidth = mapSizeWidth * tileResolution * (int)xx;
        int textHeight = mapSizeHeight * tileResolution * (int)zz;
        Texture2D texture = new Texture2D(textWidth, textHeight);
