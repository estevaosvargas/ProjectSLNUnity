...
public class TDMap
{
    ...
    public TDMap()
    {
        new TDMap(50, 50, 4, 16, 10, 10, 0.725f, 0 ,0); // default it
    }

    List<TDRoom> rooms;
    public TDMap(int mapWidth, int mapHeight, int minRoomSize, int maxRoomSize, int totalRooms, int seed, float noiseFactor, float xx, float yy)
    {
        this.mapWidth = mapWidth;
        this.mapHeight = mapHeight;
        Vector2 shift = new Vector2(xx, yy + seed);
        float zoom = 0;
        ...
        for (int x = 0; x < mapWidth; x++)
        {
            for (int z = 0; z < mapHeight; z++)
            {
                ...
                else if (perlin > 0.2f && perlin <= 0.7f)
                {
                    if (perlin > 0.2f && perlin < 0.6f)
                    {
                        ...
                        if (Random.Range(1, 128) > 125)
                        {
                            ...
                            /*GameObject obj = GameObject.Instantiate(TGMap.current.Tree, new Vector3(x, 0.1f, z), Quaternion.identity);
                        obj.transform.Rotate(new Vector3(90, 0, 0));
                        TGMap.current.Listaobj.Add(obj);*/
                        }
                        else if(Random.Range(1, 128) > 125)
                        {
                            map_data[x, z] = TDTile.TREE;
                            /*GameObject obj = GameObject.Instantiate(TGMap.current.Tree2, new Vector3(x, 0.1f, z), Quaternion.identity);
                        obj.transform.Rotate(new Vector3(90, 0, 0));
                        TGMap.current.Listaobj.Add(obj);*/
                        }

                        if (Random.Range(1, 100) > 250)
                        {
                ...
                else if (perlin > 0.7f && perlin <= 0.8f)
                {
                    ...
                    {
                        //pe do morro
                        map_data[x, z] = TDTile.STONEBlEND;
                    }
