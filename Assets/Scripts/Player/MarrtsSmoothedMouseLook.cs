...
public class MarrtsSmoothedMouseLook : MonoBehaviour {
	...
	void Update ()
	{
		...
		if (MouselockFake.IsLock) { return; }	//active?

		//Update input
		UpdateMouseBuffer();
		targetTrans.rotation = Quaternion.Euler(0, mouseBuffer.curAbs.y,0);
        transform.rotation = Quaternion.Euler(mouseBuffer.curAbs);
    }
	...
	public void ApplayGunRollBack(float ForceRollX, float ForceRollY, bool reverse)
	{
		float forcx = 0;
		float forcy = 0;
		...
		float rawPitchDelta = vSens * forcy;
